...
describe("/loop command", () => {
  ...
  it("schedules a recurring prompt with a leading interval", async () => {
    const result = loop.execute({ remainder: "5m check deployment", agent } as any);
  ...
  it("uses the default interval when no interval is provided", async () => {
    const result = loop.execute({ remainder: "check deployment", agent } as any);
  ...
  it("does not start the scheduler again when it is already running", async () => {
    (agent.getState as ReturnType<typeof mock>).mockReturnValue({ abortController: new AbortController() } as any);

    loop.execute({ remainder: "check deployment every 2h", agent } as any);
